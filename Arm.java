...
public class Arm
{
    ...
    public void inverseKinematic(double xt_new,double yt_new){
        ...
        double d1 = Math.hypot(xm1-xt, ym1-yt);
        if (d1>2*r){
        ...
        // elbows positions
        double xb = xt+0.5*(xm2-xt);
        double yb = yt+0.5*(ym2-yt); 
        
        xj1 = xb+h1*Math.cos(Math.PI/2-alpha);
        yj1 = yb+h1*Math.sin(Math.PI/2-alpha);

        theta1 = Math.atan(yj1-ym, xj1-xm);
        ...
        double dx2 = xt - xm2; 
        double dy2 = yt - ym2;
        double d2 = Math.hypot(xm2-xt, ym2-yt);
        if (d2>2*r){
        ...
        // elbows positions
        xj2 = xb - h*Math.cos(Math.PI/2-alpha);
        yj2 = yb + h*Math.sin(Math.PI/2-alpha);
        // motor angles for both 1st elbow positions
        theta2 = Math.atan(yj2-ym, xj2-xm);
